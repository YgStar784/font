...
const formulaTemplates = ref([
    { name: '模版一', content: 'A + B' },
    { name: '模版二', content: '(A + B) * C' },
    // 可以根据需要添加更多模版
]);
...
const drawArrowLine = (x1, y1, x2, y2, circleContent = '+') => {
    ...
    ctx.value.fillStyle = 'gray'; // 字符的颜色
...
const drawCanvas = () => {
    ...
    componentsOnCanvas.value.forEach((component) => {
        if (hoverComponent.value === component) {
            ...
            gradient.addColorStop(0, '#f0f8ff'); // 浅蓝色
            gradient.addColorStop(1, '#add8e6'); // 更浅的蓝色
        ...
        if (component.type != 'operators') {
            ...
            gradient.addColorStop(1, '#add8e6'); // 渐变颜色
            ...
            // 绘制用户组件的圆角矩形，圆润化
            drawRoundedRect(ctx, component.x, component.y, component.width, component.height, 20); // 更大的圆角
            ...
            let displayText = component.value.username;

            ctx.value.fillText(displayText, component.x + component.width / 2, component.y + component.height / 2);
            ...
            if (component.bracket) {
                ...
                // 根据括号类型调整括号的位置（左括号在左边，右括号在右边）
                if (component.bracket === '(') {
                    ctx.value.fillText(component.bracket, component.x - 15, component.y + component.height / 2); // 在左侧绘制左括号
                } else if (component.bracket === ')') {
                    ctx.value.fillText(component.bracket, component.x + component.width + 15, component.y + component.height / 2); // 在右侧绘制右括号
                }
            }
        } else {
...
const drawSelectionBox = (ctx, component) => {

    ctx.value.setLineDash([5, 5]); // 设置虚线
    ctx.value.strokeRect(component.x - 5, component.y - 5, component.width + 10, component.height + 10); // 绘制选中框
    ctx.value.setLineDash([]); // 重置虚线为实线
};
...
const wrapText = (ctx, text, x, y, maxWidth, lineHeight) => {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        testWidth = ctx.value.measureText(testLine).width;
...
const handleMouseEnter = () => {
    ...
    if (queryFormUsers.value.queryName === '') {
        ...
        onsole.log('mouse in', queryFormUsers.queryName, tempUserList.value, currentUsersList.value);
    }
}
const handleMouseLeave = () => {
    ...
    if (queryFormUsers.value.queryName === '') {
        ...
        onsole.log('mouse out', queryFormUsers.queryName, tempUserList.value, currentUsersList.value);
    }
}
