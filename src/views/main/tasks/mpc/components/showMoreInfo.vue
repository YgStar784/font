<template>
    ...
    <div style="padding: 20px;box-sizing: border-box;padding-top: 0;">
        ...
        <br>
        <el-card>
            <el-descriptions :column="6" v-loading="playerInfoLoading" title="参与者信息">
                <div v-for="(item, index) in taskPlayerList">
                    <el-descriptions-item :span="1" :label='`参与者 ${index}-IP`'>{{ item.playerIp
                        }}</el-descriptions-item>
                    <el-descriptions-item :span="1" label='数据描述'>{{ item.dataDescription
                        }}</el-descriptions-item>
                    <el-descriptions-item :span="2" label="Uuid">{{ item.invitationUuid }}</el-descriptions-item>
                    <!--                     <el-descriptions-item label="字段">{{ item.fieldName }}</el-descriptions-item>
 --> <el-descriptions-item :span="1" label="内网计算IP">{{ item.privateComputingIp }}</el-descriptions-item>
...
const handleClose = () => {
    ...
    router.push({ name: 'arithmetic' })
}
const isOutcomeCompleted = ref('')
...
const handleDownLoad = async (uuid) => {
    ElMessageBox.confirm(
        ...
        .then(async () => {
            ...
            axios.post('/api/MPC/downloadResultByUuid', { taskUuid: uuid }
                , {
                    headers: {
...
onMounted(async () => {
    ...
    isOutcomeCompleted.value = localStorage.getItem('isOutcomeCompleted')
    ...
    await axios.post('/api/MPC/getMyTaskPlayers', {
        uuid: taskUuid.value,
        page: 1,
        pageSize: 30
    }
