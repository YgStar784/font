<template>
 ...
 --> </template>
                    ...
                    <el-table :data="tableDataJoin" stripe
                        ...
                        <el-table-column fixed="right" label="操作" width="160px" align="center">
                            <template #default="{ row }">
                                <el-button v-if="row.state === 0" @click="handleDataSourceInfo(row)" type="success"
...
import pirAddDialog from './components/stealthAddDialog.vue'
...
import { taskOptionsJoin } from '../taskOptionsJoinPIR'
import PlayerDialog from './components/playerInfoDialog.vue'
import DataSourceInfoDialog from './components/playerInfoDialog.vue'
...
const taskInfo = ref({})
const dialogVisibleDataSouce = ref(false)
const queryForm = ref({
    queryName: '',
    page: 1,
    pageSize: 2
...
const handleDataSourceInfo = (row) => {
    dialogVisibleDataSouce.value = true
    taskInfo.value = row
}
...
const getMyTaskJoin = async () => {
    await axios.post('/api/PIR/getMyTaskInvitations', queryFormJoin.value
        , {
            headers: {
                Authorization: localStorage.getItem('token'),
            }
